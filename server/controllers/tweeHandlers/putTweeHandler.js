...
const putTweeHandler = async (req, res) => {
	try {
		// Connection URL
		...
		const caravans = await collection.updateOne(
			{ _id: oldCaravanInfo.name },
			newCaravanInfo
		)

		if (caravans.acknowledged == true)
			return res.status(200).send({ data: caravans })
